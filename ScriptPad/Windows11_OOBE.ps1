...
If (!(Test-Path $PantherUnattendPath)) {
    New-Item -Path $PantherUnattendPath -ItemType Directory -Force | Out-Null
}

# Save to Panther\Unattend
$UnattendXml | Out-File -FilePath "$PantherUnattendPath\Unattend.xml" -Encoding utf8 -Force

# Also save to base Panther as backup
$UnattendXml | Out-File -FilePath "C:\Windows\Panther\Unattend.xml" -Encoding utf8 -Force
